...
namespace Library
{
  public class Book
  {
    ...
    public List<Copy> GetCopies()
    {
      ...
      SqlCommand cmd = new SqlCommand("SELECT copies.* FROM copies WHERE book_id = @BookId;", conn);
      ...
      while (rdr.Read())
      {
        ...
        Copy newCopy = new Copy(bookId, copyStatus, copyId);
        copies.Add(newCopy);
      }
      if (rdr != null)
      {
    ...
    public List<Copy> GetAvailableCopies()
    {
      ...
      SqlCommand cmd = new SqlCommand("SELECT copies.* FROM copies WHERE book_id = @BookId AND status = 'false';", conn);
